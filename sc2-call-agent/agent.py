        ...
        req = api.RoomCompositeEgressRequest(
            ...
            segment_outputs=[api.SegmentedFileOutput(
                ...
                azure=api.AzureBlobUpload(
                    account_name=os.getenv("AZURE_STORAGE_ACCOUNT", "ringstoragesagnik"),
                    # Container name where recordings will be stored
                    account_key=os.getenv("AZURE_STORAGE_KEY", "your_azure_storage_key_here"),
                )
            )],
        )
...
def store_egress_data_in_mongo(cust_name, phone_number, customer_id, egress_data, product_name):
    ...
    storage_account = os.getenv("AZURE_STORAGE_ACCOUNT", "your_storage_account")
...
async def on_shutdown(egress_id, cust_name, phone_number, cust_details, product_details, language_call):
    ...
    await download_and_store_egress_data(
        ...
        f"https://{os.getenv('AZURE_STORAGE_ACCOUNT', 'your_storage_account')}.blob.core.windows.net/recordings/{egress_id}.json",
